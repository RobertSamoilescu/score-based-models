""" Copyright LEFTeyex/warmup
https://github.com/LEFTeyex/warmup/blob/master/src/warmup_scheduler_pytorch/warmup_module.py
"""

from torch.optim import Optimizer
from torch.optim.lr_scheduler import LRScheduler, ReduceLROnPlateau, _LRScheduler
...
class WarmUpScheduler(object):
    ...
    def __init__(
        ...
        for idx, group in enumerate(optimizer.param_groups):
            if "initial_lr" not in group:
                raise KeyError(
                    "param 'initial_lr' is not specified " f"in param_groups[{idx}] when resuming an optimizer"
                )
    ...
    def get_warmup_lr(self):
        ...
        if self.warmup_mode == "linear":
            ...
            raise ValueError("Now the other warmup_mode is not implemented, " "there is only 'linear' mode")
    ...
    def print_lr(is_verbose, group, lr, epoch=None):
        ...
        if is_verbose:
            if epoch is None:
                print(f"Adjusting learning rate " f"of group {group} to {lr:.4e}")
            else:
                print(f"Epoch {epoch:5d}: adjusting learning rate " f"of group {group} to {lr:.4e}")
